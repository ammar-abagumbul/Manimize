...
from manim.mobject.graph import Axes
...
def make_histogram(data: np.ndarray, bins: int, colors: list[Colors] | None = None) -> BarChart:
    """
...
class ConvergenceScene(Scene):
    ...
    def construct(self):
        ...
        for name, hist in histograms.items():
            ...
            self.wait(1)
            self.play(
                TransformMatchingTex(hist, normal_curve, path_arc=PI / 2),
                Transform(label, Tex("Gaussian").set_color(YELLOW)),
                run_time=2,
            )
            self.wait(1)
...
class FormalStatementScene(Scene):
    ...
    def construct(self):
        ...
        eq = MathTex(
            ...
            r"N\!\bigl(0, \sigma^{2}\bigr)."
        )
        eq.arrange(RIGHT, buff=0.2).scale(0.9).to_edge(UP, buff=1)
        ...
        conditions = BulletedList(
            r"$(X_i)_{i\ge1}$ are i.i.d.",
            r"$\mathbb{E}[X_i]=\mu$",
            r"$\operatorname{Var}(X_i)=\sigma^{2}<\infty$",
            buff=0.25,
        ...
        walk_curve = always_redraw(
            lambda: walk_axes.plot_line_graph(
                np.arange(0, int(walk_axes.x_range[1])),
                np.cumsum(np.random.choice([-1, 1], size=int(walk_axes.x_range[1]))),
...
class UbiquityScene(Scene):
    ...
    def construct(self):
        ...
        split.arrange_in_grid(cols=2, cell_alignment=DOWN, buff=1).shift(DOWN * 0.5)
...
class ApplicationsScene(Scene):
    ...
    def construct(self):
        ...
        ci_curve = always_redraw(
            lambda: ci_axes.plot_line_graph(
                np.arange(10, 500, 5),
                np.array([ci_width() for _ in np.arange(10, 500, 5)]),
                add_vertex_dots=False,
                line_color=YELLOW,
            )
        )
        ...
        dot = always_redraw(
            lambda: Dot().move_to(ci_axes.c2p(n_tracker.get_value(), ci_width()))
        )
...
class CLTVideo(Scene):
    ...
    def construct(self):
        # 1. Intuition
        self.add(IntuitionScene())
        self.wait(1)

        # 2. Adding / Summation
        self.add(SummationScene())
        self.wait(1)

        # 3. Convergence to Gaussian
        self.add(ConvergenceScene())
        self.wait(1)

        # 4. Formal statement
        self.add(FormalStatementScene())
        self.wait(1)

        # 5. Ubiquity in real data
        self.add(UbiquityScene())
        self.wait(1)

        # 6. Applications
        self.add(ApplicationsScene())
        self.wait(1)
