...
class LLMProvider:
    ...
    def invoke(self, prompt, schema: Dict | None = None) -> BaseMessage:
        if schema is None:
            return self.llm.invoke(prompt)
        else:
            structured_model = self.llm.with_structured_output(schema, method="json_schema")
...
def manim_code_generation_node(state: AppState, llm_provider: LLMProvider) -> AppState:
    ...
    code_data = parser.parse(str(response.content))
    state.manim_code = code_data["code"]
    return state


def judge_adherence_node(state: AppState, llm_provider: LLMProvider) -> AppState | None:
...
def build_graph(llm_provider: LLMProvider) -> CompiledStateGraph:
