...
def main():
    # Step 1: Crawl and save for inspection
    crawler = WebCrawler(rate_limit=1.0)
    crawler.set_target_tags(["section"])
    crawler.set_stopping_condition(lambda url: 'reference' in url) # Crawl as long as the URL contains the word 'reference'.

    inspector = DataInspector("./my_scraped_data")
    
    # Crawl and save all pages for manual inspection
    start_url = BASE_URL + STARTING_PAGE
    url = "https://docs.manim.community/en/latest/reference/manim.mobject.vector_field.VectorField.html"
    saved_files = inspector.save_all_for_inspection(crawler, url, depth=100)
    
    # Step 2: Manual inspection (you would do this manually)
    # print("\n=== MANUAL INSPECTION PHASE ===")
    # pending_files = inspector.get_pending_files()
    
    # for filename in pending_files[:2]:  # Example: inspect first 2 files
    #     preview = inspector.preview_file(filename)
    #     print(f"\nPreviewing: {filename}")
    #     print(f"URL: {preview['url']}")
    #     print(f"Title: {preview['title']}")
    #     print(f"Word Count: {preview['word_count']}")
    #     print(f"Content Preview: {preview['content_preview'][:200]}...")
        
    #     # Simulate approval (in practice, you'd manually decide)
    #     inspector.approve_file(filename)
    
    # # Step 3: Process approved files into RAG system
    # print("\n=== RAG PROCESSING PHASE ===")
    # rag_processor = RAGDataProcessor(collection_name="my_content")
    # processed_count = rag_processor.process_approved_files(inspector)
    
    # Step 4: Test search
    # if processed_count > 0:
    #     results = rag_processor.search("machine learning", n_results=2)
